...
interface EventListProps {
  ...
  onLeaveEvent?: (event: EventType) => void; // ✅ Ajouter onLeaveEvent
  currentUserId?: string; // ✅ Ajouter currentUserId
}
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.map((event) => {
          ...
          const isParticipant = event.users.some(user => user.id === currentUserId);
