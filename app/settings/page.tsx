...
function TabCompte({ onDeleteAccount }: { onDeleteAccount: () => void }) {
  ...
  return (
    <div>
      ...
      <div className="border-t border-[var(--color-grey-two)]">
        <ToggleRow label="Synchroniser votre calendrier" enabled={syncCalendar} onChange={setSyncCalendar} />
      </div>
  ...
  return (
    <div>
      ...
      <div className="border-t border-[var(--color-grey-two)]">
        ...
        <SettingRow label="Nous Contacter" value="Voir" onClick={() => router.push("/contact-us")} />
      </div>
    </div>
  );
}
  ...
  const confirmDeleteAccount = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast({ title: "Erreur", body: "Impossible de supprimer le compte." });
      }
    } catch {
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl p-8 max-w-sm w-full mx-4 shadow-xl">
            <h2 className="text-h3 font-poppins text-[var(--color-text)] mb-3">Supprimer le compte</h2>
